...
class DebouncedCallbackTest(TransactionTestCase):
    """Test debounced event callbacks"""
    ...
    def test_debounce_registration_validation(self):
        """Test that debounce requires both key and window"""
        from django_ai.automation.events.callbacks import callback_registry

        def my_callback(events):
            pass

        # Must provide both debounce_key and debounce_window
        with self.assertRaises(ValueError) as ctx:
            callback_registry.register(
                my_callback,
                debounce_key=lambda e: "key",
    ...
    def test_debounce_cannot_have_offset(self):
        """Test that debounced callbacks cannot have an offset"""
        from django_ai.automation.events.callbacks import callback_registry
    ...
    def test_debounced_callback_batches_events(self):
        """Test that debounced callback receives batched events"""
        from django_ai.automation.events.callbacks import on_event_debounced
        from django_ai.automation.testing import time_machine

        received_events = []

        @on_event_debounced(
            debounce_key=lambda e: "all",  # Group all events together
            debounce_window=timedelta(seconds=1),
            event_name="booking_confirmed",
        )
        def handle_bookings(events):
        ...
        with time_machine() as tm:
            ...
            booking3 = TestBooking.objects.create(
                guest_name="Guest 3",
                checkin_date=timezone.now() + timedelta(days=1),
                checkout_date=timezone.now() + timedelta(days=2),
                status="confirmed",
            ...
            # Events created but callback not yet fired
            self.assertEqual(len(received_events), 0)

            # Advance past debounce window
            tm.advance(seconds=2)

            # Should receive all 3 events in one batch
            self.assertEqual(len(received_events), 3)

    def test_debounced_callback_groups_by_key(self):
        """Test that debounce_key groups events separately"""
        from django_ai.automation.events.callbacks import on_event_debounced
        from django_ai.automation.testing import time_machine
        ...
        @on_event_debounced(
            debounce_key=lambda e: e.entity.guest_name.split()[0],  # Group by first word of name
            debounce_window=timedelta(seconds=1),
            event_name="booking_confirmed",
        )
        def handle_bookings(events):
            received_batches.append(list(events))  # Copy to preserve

        with time_machine() as tm:
            # Create bookings with different name prefixes (all confirmed to trigger event)
            TestBooking.objects.create(
            ...
            # Should receive 2 batches (one for "VIP", one for "Regular")
            self.assertEqual(len(received_batches), 2)

            # Check batch sizes
            batch_sizes = sorted([len(b) for b in received_batches])
            self.assertEqual(batch_sizes, [1, 2])  # 1 Regular, 2 VIP

    def test_debounced_callback_extends_window(self):
        """Test that new events extend the debounce window"""
        from django_ai.automation.events.callbacks import on_event_debounced
        from django_ai.automation.testing import time_machine
        ...
        @on_event_debounced(
            debounce_key=lambda e: "all",
            debounce_window=timedelta(seconds=2),
            event_name="booking_confirmed",
        )
        def handle_bookings(events):
            received_events.extend(events)
        ...
        with time_machine() as tm:
            ...
            # Advance 1 second (not past window)
            tm.advance(seconds=1)
            self.assertEqual(len(received_events), 0)

            # Create second booking (extends window)
            TestBooking.objects.create(
                guest_name="Guest 2",
                checkin_date=timezone.now() + timedelta(days=1),
                checkout_date=timezone.now() + timedelta(days=2),
            ...
            # Advance 1.5 more seconds (2.5 total, but window was extended)
            tm.advance(seconds=1.5)
            self.assertEqual(len(received_events), 0)
