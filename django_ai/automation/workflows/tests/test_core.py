...
class TestWorkflowValidation(WorkflowTestCase):
    ...
    def test_missing_start_step(self):
        ...
        self.assertIn("must have exactly one step marked with start=True", str(cm.exception))
    ...
    def test_invalid_goto_target(self):
        ...
        @workflow("invalid_goto", default_retry=Retry(max_attempts=1))  # No retries for this test
...
class TestWorkflowStates(WorkflowTestCase):
    ...
    def test_sleep_state(self):
        ...
        # Should be in waiting state with wake_at set
    ...
    def test_wait_for_signal(self):
        """Test wait for signal functionality"""
        ...
        class WaitWorkflow:
            ...
            @step(start=True)
            def wait_step(self):
                return wait.for_signal("test_signal")
        ...
        run = engine.start("test_wait")
        
        # Execute the wait step
        engine.execute_step(run.id, "wait_step")
        run.refresh_from_db()
        
        # Should be waiting for signal
        self.assertEqual(run.status, WorkflowStatus.WAITING)
        self.assertEqual(run.waiting_signal, "test_signal")
        self.assertIsNone(run.wake_at)  # No timeout
        
        # Send the signal
        engine.signal("test_signal")
